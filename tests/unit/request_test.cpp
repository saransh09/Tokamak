...
TEST_CASE("Request stores id, priority, and output-token budget", "[request]") {
  ...
  Request request("req-001", clock, 2000ms, /*max_output_tokens=*/128,
                  /*priority=*/3);
...
TEST_CASE("deadline_at is fixed at construction and does not drift",
          "[request]") {
  tokamak::FakeClock clock;
...
TEST_CASE("emit_token increments the counter and records first-token time",
          "[request]") {
  tokamak::FakeClock clock;
...
TEST_CASE("Request stores prompt_token_ids when constructed with them",
          "[request]") {
  tokamak::FakeClock clock;
  Request request("req-005", clock, 2000ms, 128,
                  std::vector<std::uint32_t>{10, 20, 30});

  REQUIRE(request.prompt_token_ids() == std::vector<std::uint32_t>{10, 20, 30});
  REQUIRE(request.priority() ==
          0); // still defaults correctly with the new overload
}
